...
test('phase 13 metadata and schema are active', () => {
  assert.equal(build.semver, '2.0.0-alpha.47');
  assert.equal(build.phase, '32');
  assert.equal(build.saveSchemaVersion, 26);
});
...
test('new saves include strategy block without breaking career and logistics', () => {
  ...
  assert.equal(save.meta.schemaVersion, 26);
...
test('legacy phase 12 save migrates to current schema 10', () => {
  ...
  assert.equal(migrated.meta.schemaVersion, 26);
