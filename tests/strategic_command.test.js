...
test('phase 13 metadata and schema are active', () => {
  assert.equal(build.semver, '2.0.0-alpha.28');
  assert.equal(build.phase, '13');
  assert.equal(build.saveSchemaVersion, 7);
});
...
test('new saves include strategy block without breaking career and logistics', () => {
  ...
  assert.equal(save.meta.schemaVersion, 7);
...
test('legacy phase 12 save migrates to current schema 7', () => {
