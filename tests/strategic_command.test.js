...
test('phase 13 metadata and schema are active', () => {
  assert.equal(build.semver, '2.0.0-alpha.36');
  assert.equal(build.phase, '21');
  assert.equal(build.saveSchemaVersion, 15);
});
...
test('new saves include strategy block without breaking career and logistics', () => {
  ...
  assert.equal(save.meta.schemaVersion, 15);
...
test('legacy phase 12 save migrates to current schema 10', () => {
  ...
  assert.equal(migrated.meta.schemaVersion, 15);
