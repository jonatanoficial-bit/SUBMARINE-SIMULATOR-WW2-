...
def main() -> int:
    ...
    def enter_gameplay(page) -> None:
        ...
        page.locator('[data-action="open-briefing"]:not([disabled])').first.click()
        page.locator('[data-action="start-mission"]').click()
        try:
            page.locator('.gameplay-screen').wait_for(timeout=4500)
        except Exception:
            page.locator('[data-action="start-mission"]').click()
            page.locator('.gameplay-screen').wait_for(timeout=12000)
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            rp = restored["physics"]
            record("Operation autosave restores all critical gauges", abs(restored["depth"]-72) < .2 and abs(rp["orderedDepth"]-105) < .01 and abs(rp["battery"]-63) < .02 and abs(rp["fuel"]-84) < .02 and abs(rp["oxygen"]-77) < .02 and abs(rp["co2"]-19) < .02 and abs(rp["ballast"]-67) < 2.1 and abs(rp["trim"]-4) < .7, str(rp))
            ...
            # Reuse the active gameplay page for tiny viewport checks to avoid flaky Playwright context disposal in headless CI.
            page.set_viewport_size({"width": 320, "height": 568})
            record("320x568 gameplay has no horizontal overflow", no_overflow(page))
            tiny_primary = page.locator('#open-periscope').bounding_box()
